...
class PeriodPicker {
  ...
  void saveEditedDays(Set<DateTime> selectedDates, Set<DateTime> oldPeriodSet) {
    ...
    //
    deselectedPeriodDates = oldPeriodSet.difference(selectedDates).toList()
      ..sort();
...
//Process periodDays into database
    ...
    await db.transaction((txn) async {
      for (int i = 0; i < affectedMonthsStart.length; i++) {
        await txn.delete(
          'Cycle',
          where:
              '((startDate BETWEEN ? AND ?) OR (endDate BETWEEN ? AND ?)) AND userId = ?',
          whereArgs: [
  ...
  Future<void> savePeriodDays(List<Cycle> startdateAndEnddateCyclerecords,
...
//#### Process PeriodDays ####
        for (int j = 0; j < cycles[i].length; j++) {
          ...
          PeriodDay? existing = await DatabaseHelper()
          ...
          if (curDate == curCycle.startDate &&
            ...
            if (existing != null) {
              ...
              if (!existing.isPeriodStartDay) {
                ...
                await DatabaseHelper()
                    .updatePeriodDay(updated, userId, txn: txn);
              }
            } else {
...
//corresponding Day table isPeriodDay flag to false.
  ...
  DateTime normalizeDate(DateTime startDate) {
